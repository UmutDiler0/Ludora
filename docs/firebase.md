...
3. **A real implementation** — swapped in by changing one import in the consuming code. Live for auth and rooms; not yet written for presence/connectivity or chat.

| Boundary | Interface | Mock | Real (active where noted) |
| --- | --- | --- | --- |
...
| Connectivity + room presence | `ConnectivityProbe`, `PresenceGateway` — [services/network/types.ts](../src/services/network/types.ts) | `httpProbe` (a 204 fetch, proxying "is the internet up" for "can we reach Ludora"), `mockPresence` (in-memory pub/sub, silent on a real device) | RTDB `.info/connected` for the probe, RTDB `onDisconnect()` for presence — not written yet |
| Chat | `ChatGateway` — [services/chat/types.ts](../src/services/chat/types.ts) | `mockChat` — in-memory loopback, `send` calls straight back out through `subscribe` | RTDB push under `rooms/{roomId}/chat/{channel}`, read via `child_added` — not written yet |
| Room directory | `RoomGateway` — [services/rooms/types.ts](../src/services/rooms/types.ts) | `roomGateway` (mockRooms.ts) — in-memory `Map<code, Room>`, module-level like `mockPresence`; kept for local dev, no longer imported by any screen | `roomGateway` (firebaseRooms.ts) — **wired and live**. RTDB `/rooms`, `/room_codes/{CODE}`, `/public_rooms/{roomId}` (§6.4), see §3.11 |
...
- **Real, wired:** [services/rooms/firebaseRooms.ts](../src/services/rooms/firebaseRooms.ts)'s `createFirebaseRooms()`, exported as the `roomGateway` singleton every `*-setup.tsx`/`*-lobby.tsx` and `find-room.tsx` import (repointed from `mockRooms.ts` in the same change that wired this). Restores the `/room_codes/{CODE} → roomId` indirection the mock skips, generates codes via a live existence check against RTDB (retried up to 10 times) rather than a synchronous `Map.has`, and writes `/rooms/{roomId}`, `/room_codes/{code}`, and — for public rooms only — `/public_rooms/{roomId}` in one multi-path `update()` so a room is never left half-registered. Two divergences from §6.4, both because there is no Cloud Functions deployment to own them yet (also on the file's own header): `/public_rooms/{roomId}` holds the full `Room` record rather than the minimal server-maintained projection §6.4 specifies, and there's still no idle sweep (below).
- **The RTDB instance URL is hardcoded** (`RTDB_URL` in `firebaseRooms.ts`) rather than read from config — enabling Realtime Database in the console does not add its URL to `google-services.json`/`GoogleService-Info.plist` the way `storage_bucket` is baked in at project creation, confirmed in `@react-native-firebase/database`'s own source (`getDatabase(app, url)` falls back to `app.options.databaseURL`, which is `null` without it). Re-enabling or migrating to a different RTDB instance means updating this constant by hand.
- **`mockRooms.ts` kept, not deleted**, unlike `mockAuth.ts` — no screen imports it any more, but it's still useful for local dev without a network round-trip to RTDB. It has the same simplification the real gateway doesn't: it collapses `roomId` and `CODE` into one key (no `roomId` concept — `code` doubles as the lookup key), and "browsing rooms" only ever lists rooms *that mock instance* created (there's no second player to fill a seat locally) — the same restraint `mockPresence`'s own header describes. Neither limitation applies to `firebaseRooms.ts`: a room it creates is reachable from any device, for real.
- **Every game's `*-setup.tsx` calls `roomGateway.createRoom()`** on "Continue to Lobby", passing a `RoomVisibility` chosen from `RoomVisibilityCard` ([features/games/core/RoomVisibilityCard.tsx](../src/features/games/core/RoomVisibilityCard.tsx)); the corresponding `*-lobby.tsx` calls `closeRoom()` when the owner starts the game. `stores/rooms.ts` is a thin reactive mirror of `listPublicRooms()` for the Find Room screen ([app/find-room.tsx](../src/app/find-room.tsx)). All of it is `await`ed — see the `RoomGateway` async note in §2.
- **No idle sweep.** ARCHITECTURE §7.2's 5-minute empty-room cleanup isn't implemented in either gateway — a room a user abandons mid-setup stays listed/joinable indefinitely now that this is live on a real backend, not just reset by an app restart. Needs a scheduled Cloud Function (client code can't reliably detect "everyone left"); tracked as an open item in §4 rather than faked client-side.
